...
## 当前进度与下一步
- 流程位置：grill-with-docs → SPEC → to-tickets → **实现中**（T01 ✅，`d889f56`）
- **下一步：T02 存储层**（无外部依赖，纯代码）——需先加 `vitest` 开发依赖并写单测
- T03 信令服务：需要用户 Cloudflare 账号（人类步骤，用户已知晓，账号状态未确认）
- 依赖图：T02 ← T04 ← T05 ← T06；T04 ← T07；T05/06/07 → T08（详见各票）
