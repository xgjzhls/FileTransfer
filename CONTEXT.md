...
- 文件夹发送（T18/T19）：iOS Safari 18.4+ / Android Chrome 用 `webkitdirectory` 选文件夹（桌面 Chrome 维持 File System Access）；接收端导出三种——「导出 zip（deflate level 6 均衡压缩，保留目录结构）」/「导出到文件夹…（桌面 FSA 选目标目录，无需解压）」/「批量分享（收进一个文件夹，子目录拍平）」，导出不设大小上限（T22）
