...
- **iOS 存储分区**：iOS 16.4+ 的「添加到主屏幕」PWA 与 Safari 标签页**存储相互独立**（OPFS/IndexedDB 不共享）——spike 实测：PWA 分区里占 60GB，Safari 分区看到 0。正式版需明确数据存放与清理路径都在同一模式；另外 iOS `navigator.storage.estimate()` 恒返回 0，不能依赖
