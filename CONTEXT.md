...
| 项 | 选择 | 理由 |
|---|---|---|
| 应用形态 | 单套 PWA（React 或 Svelte + TS + Vite + vite-plugin-pwa），所有端同一套代码 | 零安装、零分发；一次引导永久离线 |
...
2. 引导：是否接受「每台设备需联网一次」；空气隔离网络需电脑本地 HTTPS + 自签证书备用路线（体验差，待确认）
3. 续传交互：中断后重新扫码续传是否可接受
4. 支持的最低 iOS 版本（OPFS 15.2+；Wake Lock 17+）
5. 接收端文件去向：仅「文件」App？是否也导出到「照片」库
6. 安全模型：二维码即物理在场证明，是否需要额外 PIN
