...
- 状态：✅ 代码完成（Wake Lock 常亮 + 降级提示 + 取消/重试修复 + 分区一致性提示 + **文件夹发送（SPEC §6.3，桌面 Chrome File System Access 递归选目录）** + **容量预警（SPEC §4：桌面 estimate / iOS 写探测）**；照片门控/孤儿清理/批量队列此前已实现）；267 单测全绿；e2e 14/14（本机降级模式）
- 阻塞：T05, T06, T07
- 被阻塞者：无
